...
import { NPCNormalDialog, NPCSabioDialogAnswer } from 'src/app/interfaces/interfaces';
...
import { PopoverDialogNPCEditComponent } from '../NPC/popover-dialog-npcedit/popover-dialog-npcedit.component';
import { PopoverDialogSabioEditComponent } from '../Sabio/popover-dialog-sabio-edit/popover-dialog-sabio-edit.component';

@Component({
  selector: 'app-popover-ver-dialog-sabio-and-npc',
  templateUrl: './popover-ver-dialog-sabio-and-npc.component.html',
  styleUrls: ['./popover-ver-dialog-sabio-and-npc.component.scss'],
})
export class PopoverVerDialogSabioAndNPCComponent implements OnInit {

  razon = null;
  id: any;
  dialogsSabio: NPCSabioDialogAnswer[] = [];
  ...
  constructor(
    ...
    if (idAC) {
      this.id = idAC;
      this.razon = razon;
      this.getDialogsByACSabio(this.id, razon);
    ...
    if (id) {
      this.id = id;
      this.getDialogsByOriginSabio(this.id);
      ...
      if (razon == "accionCausa") {
        await this.firestore.firestore
          ...
          .onSnapshot((querysnap) => {
            ...
            this.totaldialogs = this.totaldialogs+querysnap.size;
  ...
  async getDialogsByACSabio(id: any, razon) {
    // show loader
    const loader = this.loadingCtrl.create({
      message: "Please wait...",
    });
  ...
  async getDialogsByOriginSabio(id: any) {
    // show loader
    const loader = this.loadingCtrl.create({
      message: "Please wait...",
    });
  ...
  async presentPopoverupdateDialogNPCPLUSSabio(idPlus: any) {
    await this.popoverCtrl.dismiss();
    const popover = await this.popoverCtrl.create({
  ...
  async presentAlertConfirmDeleteDialogPlusSabio(dialog: any) {
    const alert = await this.alertController.create({
      cssClass: "my-custom-class",
      header: "¿Confirmar Eliminacion?",
      message:
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            console.log("Confirm Okay");
            this.deleteDialogNPCPLusSabio(dialog);
          },
        },
      ],
    });
  ...
  async deleteDialogNPCPLusSabio(dialog) {
    // show loader
    console.log("Delete", dialog);
    const loader = this.loadingCtrl.create({
      message: "Please wait...",
